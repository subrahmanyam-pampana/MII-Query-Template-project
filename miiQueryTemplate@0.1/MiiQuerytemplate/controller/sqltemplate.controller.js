  ...
  function (
    ...
    return Controller.extend("app.controller.sqltemplate", {
      onInit: function () {
        ...
        this.BLSTransactions = this.loadResource("./model/BLSTransactions.json")
        ...
        //enabling parameters
        var parameterData = this.loadResource("./model/parameterData.json")
        this.byId("idParametersTable").setModel(new JSONModel(parameterData))
      },
      onAfterRendering: function () {
        var that = this
        this.byId("idTableList").onAfterRendering = () => {
          that.enableAutoComplete()
        }
        //setting key board short cuts
      ...
      setDataServersList: function () {
        ...
        if (localStorage[`${that.server.Alias}_dataServers`] !== undefined) {
          ...
          try {
            $.ajax({
              url: `${that.serversList.servers[0].url}/XMII/Runner?Transaction=${that.BLSTransactions.executeTransactionPath}&OutputParameter=*&Content-Type=text/json`,
              method: "POST",
              async: false,
              data: {
              ...
              success: function (oData) {
                //checking the errors
                if (oData.root && oData.root.status == "ERROR") {
                  MessageBox.error(oData.root.message)
                  dataServerView.setBusy(false)
                  dataServerView.setModel(new JSONModel({}))
                  return
                } else if (
                  oData.Rowsets &&
                  oData.Rowsets.FatalError !== undefined
                ) {
            ...
            $.ajax({
              ...
              success: function (oData) {
                ...
                if (oData.root && oData.root.status == "ERROR") {
                  ...
                  tablesView.setModel(new JSONModel({}))
                  return
                } else if (
                  oData.Rowsets &&
                  oData.Rowsets.FatalError !== undefined
      ...
      pasteSelectedTable: function (oEvent) {
        ...
        var selectedKey = tableView.getSelectedItem().getTitle()
      ...
      onColumnSearch: function (oEvent) {
        ...
        var sQuery = oEvent.getSource().getValue()
        if (sQuery && sQuery.length > 0) {
          var filter = new Filter("ColumnName", FilterOperator.Contains, sQuery)
          aFilters.push(filter)
        }
      ...
      onTableSelect: function (oEvent) {
        ...
        var tableName = selectedItem.getTitle() //change
        ...
        jQueryTextArea.textcomplete([
          {
            ...
            replace: function (hit) {
              var tablesView = that.byId("idTableList")
 	    var searchFiled = that.byId("idTableSearch")

              searchFiled.setValue("")
              searchFiled.fireLiveChange()

	    tablesView.getItems().forEach(item=>{ 
					if(item.getTitle()==hit.TableName){
						tablesView.setSelectedItem(item,true)
						tablesView.fireSelectionChange()
						return
					}
				})
              ...
              var searchFiled = that.byId("idTableSearch")
              searchFiled.setValue(hit.TableName)
              searchFiled.fireLiveChange()
      ...
      executeQuery: function (oEvent) {
        ...
        var paramData = that.byId("idParametersTable").getModel().getData()
        query = parameterlogic.replaceParameters(paramData, query)
        var queryExecuteUrl = `${that.server.serverName}/XMII/Illuminator?QueryTemplate=${that.BLSTransactions.executeQueryPath}&Server=${that.dataServer}&RowCount=${rowCount}&Content-Type=text/json&${that.credentialsString}`
        vBox.setBusy(true)
        try {
          $.ajax({
            url: `${that.serversList.servers[0].url}/XMII/Runner?Transaction=${that.BLSTransactions.executeTransactionPath}&OutputParameter=*&Content-Type=text/json`,
      ...
      exportToExcel: function (oEvent) {
        ...
        var aColumnsData = oTable.getBindingContext().getObject().Columns.Column
    ...
    onTableDrop: function (oEvent) {
        var tableListView = this.byId("idTableList")
        var selectedItem = tableListView.getSelectedItem()

          this.insertQueryToTextArea(oEvent.getParameter("draggedControl").getTitle() )
        
      },
      setVisibleRowCount: function (oEvent) {
        var oTable = oEvent.getSource().getParent().getParent()
        var selectedItem = oEvent.getParameters("selectedItem")
        oTable.setVisibleRowCount(parseInt(selectedItem.selectedItem.getText()))
      },
      ...
      onParameterDrop: function (oEvent) {
        parameterlogic.onParameterDrop(oEvent)
      },
